 ...
 * 観光協会登録画面コントローラ
 */
class Agyscyreg extends CI_Controller {
	/*
	 * コンストラクタ
	*/
	...
	function index()
	{
		...
		if(isset($_POST['regist'])) {
			...
			// 入力チェック
			$data['msg_user_nm'] = 	$this->cms_common->cms_validation(1,$db_user_nm, 50, '観光協会名');
			$data['msg_kana_user_nm'] = $this->cms_common->cms_validation(1,$db_kana_user_nm, 50,'観光協会名(カナ)');
			...
			if($this->_err_judge($data) == true)
			{
				...
				// エラーあり
				$this->load->view('agyscyreg_view', $data);
